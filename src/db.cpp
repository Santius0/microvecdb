...
namespace mvdb {
    ...
    std::ostream& operator<<(std::ostream& os, const DB_& obj) {
    ...
    void DB_::serialize(std::ostream &out) const {
...
//        out.write(reinterpret_cast<const char*>(_records->data()), ntotal_ * dims_ * sizeof(value_t));
        ...
        _storage->serialize(out);
    }

    void DB_::deserialize(std::istream &in) {
        ...
        _index = std::make_unique<FaissFlatIndex>(_dims, _index_path);

        _index->deserialize(in);
        _storage->deserialize(in);
    }
    ...
    DB_::Status* DB_::status() const {
        return status_.get();
    }

    bool DB_::open(const std::string &path) {
    ...
    bool DB_::create(const std::string& path, const idx_t& dims) {
        if(!fs::exists(path)) {
        ...
        _storage = std::make_unique<Storage>(_storage_path, true, false);
        _index = std::make_unique<FaissFlatIndex>(dims, _index_path);
        return true;
    }

    void DB_::_save(const std::string& save_path) {
    ...
    idx_t* DB_::add_vector(const idx_t& nv, value_t* v) {
        delete _add_ids;
...
//         return nullptr;
    ...
    void DB_::search_with_vector(const idx_t& nq, value_t* query, const long& k, idx_t* ids, value_t* distances) {
    ...
    value_t* DB_::get(idx_t& n, idx_t* keys) const {
        if(!_index) return nullptr;
