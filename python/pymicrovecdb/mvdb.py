...
class MVDB:
    ...
    def create(self, index_type: IndexType, dims: np.int64, path: str, initial_data_path: str = "", initial_data: np.array = np.array([]), initial_data_size: np.int64 = -1, **kwargs):
    ...
    def dims(self):
        ...
        return mvdb_c.MVDB_get_dims(self.data_type, self.mvdb_obj)

    def topk(self, query: np.array = np.array([]), query_file = "", k: np.uint64 = 5, num_queries: np.uint64 = -1,
